...
//!
//! `hura://host:port/<token>#<fingerprint>` -- an address, a credential and the
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let what = match self {
            ParseError::Scheme => "a pairing string starts with `hura://`",
...
impl FromStr for Pairing {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let rest = s.trim().strip_prefix("hura://").ok_or(ParseError::Scheme)?;
...
mod tests {
    ...
    fn it_round_trips_through_the_string_a_person_pastes() {
        ...
        assert_eq!(printed, format!("hura://wsl.local:17671/d0e1f2#{FP}"));
    ...
    fn a_short_or_non_hex_fingerprint_is_refused_at_the_paste() {
        for bad in [
            format!("hura://h:1/t#{}", &FP[..40]),
            format!("hura://h:1/t#{}zz", &FP[..62]),
            "hura://h:1/t".to_string(),
        ] {
            assert_eq!(
    ...
    fn the_parts_that_are_missing_are_named() {
        ...
        assert_eq!(
            format!("hura://:1/t#{FP}").parse::<Pairing>().unwrap_err(),
            ParseError::Host
        );
        assert_eq!(
            format!("hura://h:nope/t#{FP}")
                .parse::<Pairing>()
                .unwrap_err(),
            ParseError::Port
        );
        assert_eq!(
            format!("hura://h:1/#{FP}").parse::<Pairing>().unwrap_err(),
            ParseError::Token
        );
    }
    ...
    fn a_paste_survives_its_surroundings() {
        let messy = format!("  hura://h:17671/tok#{}\n", FP.to_ascii_uppercase());
    ...
    fn a_bracketed_ipv6_host_parses() {
        let p: Pairing = format!("hura://[::1]:17671/tok#{FP}").parse().unwrap();
        assert_eq!(p.host, "[::1]");
