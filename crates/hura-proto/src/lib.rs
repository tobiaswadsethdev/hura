//! The wire between a client and `hurad`.
//!
...
//! a crate to avoid.
//!
//! **The types on the wire are the core's own.** [`hura_core::session::Session`]
...
use hura_core::ops::{NewOptions, NewSession, Picked, Poll, Refreshed};
...
pub enum Request {
    ...
    /// every session on this machine, including the ones `hura new` starts, and
    /// a client keeping its own answer would be a second convention that
    /// disagrees with the first. See [`hura_core::settings`].
    Settings,
...
mod tests {
    ...
    fn nothing_on_the_wire_carries_a_secret_value_back() {
        ...
        let view = IntegrationsView {
            mcp: Vec::new(),
            secrets: vec![hura_core::secrets::Named {
                name: "SENTRY_TOKEN".into(),
                set: true,
    ...
    fn a_poll_carried_whole_round_trips_under_the_tag() {
        let out: Outcome = Reply::from(Poll {
            stat: Some(hura_core::ops::DiffStat {
                added: 12,
                removed: 3,
                untracked: 1,
            }),
    ...
    /// string, and the advice `hura doctor` gives a Windows client.
    #[test]
    ...
    fn hello_parses_without_the_fields_a_later_version_might_add() {
        let h: Hello =
            serde_json::from_str(r#"{"server":"hurad","protocol":1,"version":"0.2.0"}"#).unwrap();
