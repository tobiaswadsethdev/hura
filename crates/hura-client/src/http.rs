...
//!
//! Enough HTTP/1.1 to talk to an `hurad` and nothing else. A client crate would
...
/// `hurad` binds one address -- so every request pays for the wrong one first.
...
pub fn request(
    ...
    let server_name = ServerName::try_from(strip_brackets(host).to_string())
        .unwrap_or_else(|_| ServerName::try_from("hurad").unwrap());
