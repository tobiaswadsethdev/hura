//! Talking to an `hurad` somewhere else.
//!
//! A saved connection is a [`Pairing`] with a name on it, kept in the state
//! directory beside anything else secret this machine holds. `hura connect`
...
use hura_core::state;
use hura_proto::{Failure, Hello, Outcome, Pairing, Reply, Request};
...
impl Remote {
    ...
    pub fn hello(&self) -> Result<Hello, Error> {
        ...
        serde_json::from_str(&response.body).map_err(|_| {
            ...
            // impostor.
            Error::Server(
                "that is not a hurad, or it is one this build is too old to understand".into(),
            )
        })
    }
            ...
            401 => {
                return Err(Error::Server(format!(
                    "`{}` did not accept this token. It may have been revoked; \
                     run `hurad pair` on the server and `hura connect` here again",
                    self.name
                )));
            }
            400 => {
    ...
    pub fn select(&self, name: Option<&str>) -> Result<&Remote, String> {
        match name {
            Some(name) => self.get(name).ok_or_else(|| {
                format!("no server named `{name}`; `hura remotes` lists the paired ones")
            }),
            None => match self.remotes.as_slice() {
                [] => Err(
                    "no server is paired; run `hurad pair` there and `hura connect` here".into(),
                ),
                [only] => Ok(only),
                many => Err(format!(
...
///
/// `hura connect` is this and a `println!`, and so is the desktop application's
...
/// `hurad`, and one whose protocol this build cannot speak all produce a saved
...
/// to the host, which is what `hura connect` has always done.
pub fn pair(pairing: &str, name: Option<&str>) -> Result<(Remote, Hello), String> {
    ...
    let pairing: Pairing = pairing
        .trim()
        .parse()
        .map_err(|e: hura_proto::pairing::ParseError| e.to_string())?;
    let name = name
        .map(str::trim)
        .filter(|n| !n.is_empty())
    ...
    if !hello.speaks(hura_proto::VERSION) {
        return Err(format!(
            "`{}` speaks protocol {} and this hura speaks {}. Update whichever is older.",
            candidate.address(),
            hello.protocol,
            hura_proto::VERSION
        ));
...
pub fn checks() -> Vec<hura_core::doctor::Check> {
    let remotes = match Remotes::load() {
        Ok(r) => r,
        Err(e) => {
            return vec![hura_core::doctor::Check::warn(
                "servers",
...
fn check_one(remote: &Remote) -> hura_core::doctor::Check {
    let hello = match remote.hello() {
        Ok(h) => h,
        Err(e) => {
            return hura_core::doctor::Check::fail(
                "servers",
                format!("{}: {e}", remote.name),
                format!(
                    "check it is running, and that `{}` is the address this machine \
                     should dial. `hura remotes --forget {}` drops it",
                    remote.address(),
                    remote.name
                ),
            );
    ...
    if !hello.speaks(hura_proto::VERSION) {
        return hura_core::doctor::Check::fail(
            "servers",
            format!(
                "{}: speaks protocol {}, this hura speaks {}",
                remote.name,
                hello.protocol,
                hura_proto::VERSION
            ),
    ...
    match remote.call(Request::Ls) {
        Ok(_) => hura_core::doctor::Check::ok(
            "servers",
            format!(
                "{}: {} (hurad {})",
                remote.name,
        ...
        Err(e) => hura_core::doctor::Check::fail(
            "servers",
            format!("{}: reachable, but {e}", remote.name),
            "`hurad pair` on the server, then `hura connect` here with the new string".to_string(),
        ),
    }
}
...
mod tests {
    ...
    fn a_pairing_string_becomes_a_named_remote() {
        let pairing: Pairing = format!("hura://box.lan:17671/abc#{FP}").parse().unwrap();
    ...
    fn a_name_that_is_not_paired_says_where_to_look() {
        ...
        assert!(err.contains("hura remotes"), "{err}");
    }
    ...
    fn remotes_survive_a_round_trip_through_the_file() {
        let path = std::env::temp_dir().join(format!("hura-remotes-{}.json", std::process::id()));
    ...
    fn a_missing_file_is_no_servers_rather_than_an_error() {
        let path = std::env::temp_dir().join("hura-remotes-definitely-absent.json");
        let _ = std::fs::remove_file(&path);
