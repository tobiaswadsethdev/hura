//! `hurad` - run several coding agents in parallel, each in its own sandbox.
//!
//! One binary for the whole Linux side: the server the desktop application
//! dials, and the commands for the machine it runs on. It absorbed `hura` in
...
//! belongs to [`hura_core`], which is what lets the window and this agree.
...
struct Cli {
    ...
    /// `hurad remotes` lists them. The name has to be attached with `=`, because
    /// a detached value cannot be told apart from the subcommand -- `hurad
...
enum Command {
    ...
    ///
    ///   printf %s "$TOKEN" | hurad secret SENTRY_TOKEN
    Secret {
        name: String,
    ...
    /// Pair with an `hurad`, using the string `hurad pair` printed.
    Connect {
        /// `hura://host:port/<token>#<fingerprint>`.
        pairing: String,
    ...
    /// Update hurad itself to the newest published release.
...
fn main() -> ExitCode {
    ...
    if let Some(at) = update::apply_staged() {
        ...
        let err = std::process::Command::new(&at)
            ...
            .exec();
        eprintln!(
            "hurad: updated to {}, but could not restart: {err}",
            at.display()
        );
    }
    ...
    let result = match cli.command {
        ...
        // before that `hurad serve` was a subcommand you had to name. Serving is
...
fn cmd_connect(pairing: &str, name: Option<&str>) -> Fallible {
    // The checking and the saving are `hura_client::pair`, because the desktop
    ...
    println!(
        "paired with `{}` at {} (hurad {})",
        remote.name,
        remote.address(),
        hello.version
    );
    println!("try: hurad --server {} ls", remote.name);
    Ok(())
}
...
fn cmd_remotes(forget: Option<&str>) -> Fallible {
    ...
    if remotes.is_empty() {
        println!("no servers paired. run `hurad pair` on one, then `hurad connect <string>`");
        return Ok(());
    }
...
fn remote_ls(remote: &remote::Remote) -> Fallible {
    let hura_proto::Reply::Ls {
        sessions,
        adopted,
        dead,
        warnings,
    } = remote.call(hura_proto::Request::Ls)?
    else {
...
fn poll_state(remote: &remote::Remote, name: &str) -> Option<State> {
    match remote.call(hura_proto::Request::Poll { name: name.into() }) {
        Ok(hura_proto::Reply::Poll(poll)) => poll.status.map(|r| r.state),
        _ => None,
    }
}

fn remote_diff(remote: &remote::Remote, name: &str) -> Fallible {
    let stat = match remote.call(hura_proto::Request::Poll { name: name.into() })? {
        hura_proto::Reply::Poll(poll) => poll.stat,
        _ => None,
    };
    let hura_proto::Reply::Diff { body } =
        remote.call(hura_proto::Request::Diff { name: name.into() })?
    else {
...
fn remote_policy(remote: &remote::Remote, name: &str) -> Fallible {
    let hura_proto::Reply::Policy(view) =
        remote.call(hura_proto::Request::Policy { name: name.into() })?
    else {
...
fn remote_watch(remote: &remote::Remote, name: &str) -> Fallible {
    use hura_proto::stream::{Channel, ClientFrame, ServerFrame};
...
fn remote_events(remote: &remote::Remote, name: &str) -> Fallible {
    let hura_proto::Reply::Events { events } =
        remote.call(hura_proto::Request::Events { name: name.into() })?
    else {
...
fn cmd_new(backends: &Backends, args: NewArgs, cfg: &Config) -> Fallible {
    ...
    let draft = ops::Draft {
        ...
        // comes from. Left `None` it would be `hura/<name>` whatever the file
    ...
    for warning in mcp::ensure(cfg.mcp()) {
        eprintln!("hurad: warning: {warning}");
    }
    ...
    if !draft.start {
        println!(
            "agent not started (--no-start); attach with: hurad attach {}",
            s.name
        );
    }
...
/// what a person wants to know is what the next `hurad new` will do, and the
...
fn cmd_update(check: bool, tag: Option<&str>, force: bool) -> Fallible {
    if check {
        match update::check() {
            update::Status::Newer { running, latest } => {
                println!("hurad {running} is running; {latest} is out");
                println!("  update with: hurad update");
            }
            update::Status::Current(v) => println!("hurad {v} is the newest release"),
            update::Status::Ahead(v) => {
                println!("hurad {v} is ahead of the newest release (a build from a checkout)")
            }
            // Never "up to date": the question was not answered.
            update::Status::Unknown => {
                println!(
                    "hurad {}; could not read the release list",
                    update::current()
                )
            }
        }
    ...
    match update::install(tag, force)? {
        update::Outcome::NoChange { version } => {
            println!("hurad {version} is the newest release; nothing to do");
            println!("  reinstall it anyway with: hurad update --force");
        }
        update::Outcome::Updated { from, to, at } => {
            println!("hurad {from} -> {to}  ({})", at.display());
            ...
            println!("  restart it to pick this up: systemctl --user restart hurad");
            // The sandbox image is versioned separately and an update is the
            // moment its recipe most likely changed underneath it.
            println!("  `hurad image build` picks up any change to the sandbox image");
        }
    }
    Ok(())
}
...
fn cmd_config(cfg: &Config, init: bool, path_only: bool) -> Fallible {
    ...
    println!("{}", cfg.path.display());
    if !cfg.present {
        println!("  (no file; create one with: hurad config --init)");
    }
    println!();
...
fn print_sessions(rows: &[(Session, State)]) {
    if rows.is_empty() {
        println!("no sessions. create one with: hurad new --repo <url> --task <what to do>");
        return;
    }
...
fn require_session(name: &str) -> Result<Session, Box<dyn std::error::Error>> {
    Store::load()?
        .get(name)
        .cloned()
        .ok_or_else(|| format!("no session `{name}`; see hurad ls").into())
}
...
fn cmd_rm(backends: &Backends, names: Vec<String>) -> Fallible {
    ...
    for name in names {
        // One name at a time, each persisting as it goes: `hurad rm a b` that
...
///
/// The same [`hura_core::mcp`] calls the window's integrations screen makes, so a
...
fn mcp(action: Option<McpAction>, name: Option<String>) -> Fallible {
    let cfg = hura_core::config::Config::load()?;
    if let Some(action) = action {
        ...
        match action {
            McpAction::Start => {
                if let Some(e) = hura_core::mcp::ensure(std::slice::from_ref(entry)).first() {
                    return Err(e.clone().into());
                }
            }
            McpAction::Restart => hura_core::mcp::start(entry)?,
            McpAction::Stop => hura_core::mcp::stop(entry.name())?,
        }
    }

    let live = hura_core::mcp::statuses(cfg.mcp());
    if live.is_empty() {
...
fn list_secrets() -> Fallible {
    let names = hura_core::secrets::names();
    if names.is_empty() {
        println!("no secrets. `printf %s \"$TOKEN\" | hurad secret <NAME>` stores one.");
        return Ok(());
    }
...
fn secret(name: &str, forget: bool) -> Fallible {
    ...
    hura_core::secrets::set(name, value.trim_end_matches(['\n', '\r']))?;
    println!("stored `{name}`; restart the servers that use it: hurad mcp --action restart <name>");
    Ok(())
}

fn list_skills() -> Fallible {
    let library = hura_core::skills::library();
    if library.is_empty() {
        println!(
            "no uploaded skills in {}. A client pushes its own when it creates a session.",
            hura_core::skills::library_dir().display()
        );
        return Ok(());
    }
...
fn serve(opts: Serve) -> Fallible {
    ...
    if tokens.is_empty() {
        ...
        eprintln!("hurad: no tokens yet, so nothing can connect. Run `hurad pair` to make one.");
    }
    ...
    match hura_core::ops::refresh_with(&rpc::backends(), true) {
        Ok(r) => {
    ...
    if let Ok(cfg) = hura_core::config::Config::load() {
        let managed = cfg.mcp().iter().filter(|e| e.is_managed()).count();
        if managed > 0 {
            let warnings = hura_core::mcp::ensure(cfg.mcp());
            println!(
    ...
    let auto_update = hura_core::config::Config::load()
        .ok()
...
/// starts `hurad` again, at which point `main` swaps it in -- so a session
...
/// an update, and `hurad doctor` says so on demand anyway.
///
...
async fn watch_for_releases() {
    ...
    loop {
        match tokio::task::spawn_blocking(update::stage).await {
            Ok(Ok(Some(v))) => {
                println!(
                    "hurad {v} downloaded and waiting; it is applied the next time hurad starts"
                )
            }
            Ok(Ok(None)) => {}
            Ok(Err(e)) => eprintln!("hurad: could not check for a new release: {e}"),
            // The blocking pool cancelled or panicked. Not a reason to stop
            // checking for the lifetime of the process.
            Err(e) => eprintln!("hurad: the update check did not run: {e}"),
        }
...
fn wsl_hint() -> Option<String> {
    ...
    Some(
        "This is WSL. If the client is on Windows, the host above is probably not\n\
         ...
         default NAT use the address `hostname -I` prints. `hurad doctor` says which."
            .to_string(),
    )
}
...
fn list_tokens() -> Fallible {
    let tokens = Tokens::load()?;
    if tokens.is_empty() {
        println!("no tokens. `hurad pair` makes one.");
        return Ok(());
    }
...
fn revoke(name: &str) -> Fallible {
    ...
    match tokens.revoke(name)? {
        0 => Err(format!("no token named `{name}`; see `hurad tokens`").into()),
        1 => {
