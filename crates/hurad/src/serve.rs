...
//!
//! Two routes for now. `GET /version` answers [`hura_proto::Hello`] to anyone,
//! because a client that cannot tell whether it has reached an `hurad` at all
...
use hura_proto::{Failure, Hello, Outcome, Request};
...
pub struct Server {
    ...
    /// change can be `hurad revoke` in another terminal while a request is in
    /// flight.
...
/// client is `hura-client`, on the Rust side of the desktop application, and it
...
fn authorised(server: &Server, headers: &HeaderMap) -> bool {
    ...
    // Checked per request, so `hurad pair` reaches a running server and -- the
    // direction that matters -- `hurad revoke` does too. It is a `stat`, and
...
mod tests {
    ...
    fn paired(name: &str) -> (Tokens, String, std::path::PathBuf) {
        let path =
            std::env::temp_dir().join(format!("hurad-serve-{name}-{}.json", std::process::id()));
    ...
    /// The whole reason the set is behind a lock: `hurad pair` and `hurad revoke`
    ...
    fn a_server_with_no_tokens_accepts_nothing() {
        let path =
            std::env::temp_dir().join(format!("hurad-serve-empty-{}.json", std::process::id()));
        let _ = std::fs::remove_file(&path);
