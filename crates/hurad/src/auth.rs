...
///
/// Reloaded when the file changes, which is what makes `hurad pair` work against
/// a server that is already running -- and, the direction that actually
/// matters, makes `hurad revoke` take effect without a restart. A set read once
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let p = std::env::temp_dir().join(format!("hurad-auth-{name}-{}", std::process::id()));
    ...
    /// `hurad pair` and `hurad revoke` both run in a *different process* from
