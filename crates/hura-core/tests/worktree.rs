...
use hura_core::backend::{Backend, Torn, Worktree};
use hura_core::seed::{self, SeedState};
use hura_core::session::{Kind, Session, State};
use hura_core::{files, git, ops};
...
fn temp(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("hura-worktree-{name}-{}", std::process::id()));
...
///
/// ```sh
/// cargo test -p hura-core --test worktree -- --ignored
/// ```
///
...
fn shells_are_this_session_s_and_nobody_else_s() {
    ...
    assert_eq!(opened, "hura-mine-shell-1");
    assert_eq!(
