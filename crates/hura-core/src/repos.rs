...
///
/// `HURA_REPO_ROOTS` (colon-separated, like `PATH`) first, then `repo_roots` from
...
pub fn roots(configured: Option<&[PathBuf]>) -> Vec<Root> {
    if let Some(raw) = std::env::var_os("HURA_REPO_ROOTS") {
...
pub fn default_roots() -> Vec<Root> {
    ...
    // The working directory first: running `hura` from inside a checkout is the
...
///
/// A subsequence match with bonuses, not a substring match: typing `hura` should
...
pub fn score(needle: &str, haystack: &str) -> Option<i32> {
    ...
    for want in needle.chars() {
        ...
        // A prefix match outranks a match in the middle: for the query `hura`,
        // `hura-playground` is more likely to be what was meant than
        // `toolbox-hura`, and without this the shorter name wins on length
        // alone.
    ...
    // Shorter haystacks win ties: `~/dev/hura` should sort above
    // `~/dev/hura-playground` for the query `hura`.
...
mod tests {
    ...
    fn configured_roots_replace_the_defaults() {
        ...
        if std::env::var_os("HURA_REPO_ROOTS").is_some() {
            return;
        }
    ...
    fn scoring_matches_subsequences_and_prefers_boundaries() {
        assert!(score("hura", "hura").is_some());
        assert!(
            score("hura", "github-runner-agent").is_some(),
            "h-u-r-a in order"
        );
        assert!(score("xyz", "hura").is_none(), "no such subsequence");
    ...
    fn filtering_puts_the_shortest_exact_name_first() {
        let repos = [
            repo_named("hura-playground", "~/dev/hura-playground"),
            repo_named("hura", "~/dev/hura"),
            repo_named("toolbox-hura", "~/work/toolbox-hura"),
            repo_named("notes", "~/dev/notes"),
        ];
        let order = filter(&repos, "hura");
        assert_eq!(
            order
                .iter()
                .map(|i| repos[*i].name.as_str())
                .collect::<Vec<_>>(),
            vec!["hura", "hura-playground", "toolbox-hura"],
