...
/// chatter on stderr.
const PUSHED: &str = "@@hura-pushed@@";
const PR: &str = "@@hura-pr@@ ";
const WARN: &str = "@@hura-warn@@ ";
const FAILED: &str = "@@hura-failed@@ ";
...
pub enum Error {
    ...
    #[error(
        "the push was refused with 403. Two things cause that and the message \
         ...
         none. Check with `hura policy <session>`; the events pane shows the \
...
mod tests {
    ...
    fn a_policy_denial_names_the_rule_that_would_allow_it() {
        ...
        assert!(msg.contains("hura policy"), "{msg}");
