...
/// thing with an agent in it, and removing one is `hura rm`'s job, said out loud.
...
/// thing to have, and they cannot both be called `hura`.
...
mod tests {
    ...
    fn scratch(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("hura-projects-{tag}-{}", std::process::id()));
    ...
    fn a_project_is_named_after_its_folder_and_round_trips() {
        ...
        let all = add_at(&store, draft("/home/x/dev/hura", "https://h/o/hura.git")).unwrap();
        assert_eq!(all.len(), 1);
        assert_eq!(all[0].name, "hura");
        assert_eq!(list_at(&store)[0].repo, "https://h/o/hura.git");
    }

    /// Two checkouts of one repository is a normal thing to have, and they
    /// cannot both be called `hura`.
    #[test]
    fn a_second_checkout_of_the_same_repository_gets_its_own_name() {
        let store = scratch("dupe");
        add_at(&store, draft("/a/hura", "https://h/o/hura.git")).unwrap();
        let all = add_at(&store, draft("/b/hura", "https://h/o/hura.git")).unwrap();
        assert_eq!(all[1].name, "hura-2");
    }
    ...
    fn the_same_folder_twice_is_refused_by_name() {
        ...
        add_at(&store, draft("/a/hura", "https://h/o/hura.git")).unwrap();
        let err = add_at(&store, draft("/a/hura", "https://h/o/hura.git")).unwrap_err();
        assert!(err.contains("already the project `hura`"), "{err}");
    }
