...
fn check_version() -> Check {
    match crate::update::check() {
        crate::update::Status::Newer { running, latest } => Check::warn(
            "version",
            format!("hurad {running}; {latest} is out"),
            "hurad update",
        ),
        crate::update::Status::Current(v) => Check::ok("version", format!("hurad {v}, newest")),
        crate::update::Status::Ahead(v) => {
            Check::ok("version", format!("hurad {v}, ahead of the newest release"))
        }
        crate::update::Status::Unknown => Check::ok(
            "version",
            format!(
                "hurad {} (no release list to compare against)",
...
fn check_trackers(sources: &[tracker::Source]) -> Check {
    ...
    Check::warn(
        "trackers",
        problems.join("; "),
        "store it with `printf %s \"$TOKEN\" | hurad secret <NAME>`, or from the \
