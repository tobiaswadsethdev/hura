...
//! environment, and the *names* of secrets the store holds. `hurad` starts it,
...
/// these, and a name collision would have `hurad` restarting something that is
/// not its own.
pub const CONTAINER_PREFIX: &str = "hura-mcp-";
...
impl Entry {
    ...
    /// An entry `hurad` runs, with the URL derived from the container it will
    /// start.
    ///
...
///
/// Called when `hurad` starts and before a session is seeded. Idempotent by
...
fn inspect(container: &str) -> Inspected {
    ...
    if !out.status.success() {
        ...
        // `error: no such object: hura-mcp-probe` where older versions said
