...
//! comments are most of what `hurad config --init` writes: the file's job is to
//! document the defaults as much as to change them, and a settings screen that
//! silently deleted the documentation would make the next person's `hurad
...
/// text `hurad config --init` writes. Saving one field should not produce a
...
pub fn save(path: &Path, settings: &Settings) -> Result<Config, Error> {
    ...
    // parse is one every command except `hura doctor` refuses to run against,
...
///
/// Under its own documentation, when the file has some. `hurad config --init`
...
mod tests {
    ...
    fn every_managed_key_survives_a_round_trip_through_the_example() {
        ...
        assert!(text.contains("# hura defaults."));
    ...
    fn writes_and_reloads() {
        let dir = std::env::temp_dir().join(format!("hura-settings-{}", std::process::id()));
    ...
    fn a_rejected_value_leaves_the_file_alone() {
        let dir = std::env::temp_dir().join(format!("hura-settings-bad-{}", std::process::id()));
        let path = dir.join("config.toml");
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        fs::write(&path, "base = \"main\"\n").unwrap();
...
mod real_file {
    ...
    //! is not a fixture. Run by hand -- `cargo test -p hura-core real_file --
    ...
    fn a_tracker_round_trips_through_the_local_config() {
        ...
        // developer's file.
        let dir = std::env::temp_dir().join(format!("hura-real-tracker-{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let path = dir.join("config.toml");
