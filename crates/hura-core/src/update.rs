...
//! updating is `hurad update` rather than remembering the curl line.
//!
//! **Explicit, never in the background.** Nothing here runs on a timer and
//! nothing installs without being asked: [`check`] is what `hura doctor` calls,
...
//! anyone remembering.
//!
//! **The asset was renamed in v0.4.0**, from `hura-<tag>-<target>.tar.gz` to
//! `hurad-<tag>-<target>.tar.gz`, when `hura` was folded into `hurad`. Nothing
//! older can follow that: `hura update` replaces the binary it is running, and
//! from v0.4.0 there is no `hura` to replace it with. It fails saying the
...
//! crosses it.** Not even from v0.3.1: the `hurad` that release installs is the
...
//! reason -- it is the last release `hura update` can reach, so nobody is
...
pub const REPO: &str = "tobiaswadsethdev/hura";
...
/// The release this build of `hura` would install for itself.
///
...
/// binary on its old inode through a rename, so an `hurad` serving sessions goes
...
/// half-written `hura` on the path -- and Linux is happy to rename over a binary
...
/// the desktop application's sake -- there are no Windows releases of `hura` to
/// download and no `hura update` there to download one.
#[cfg(windows)]
...
mod tests {
    ...
    const RELEASE_JSON: &str = r#"{
      ...
      "draft": false,
      "assets": [
        {"name": "hurad-v0.2.0-x86_64-unknown-linux-musl.tar.gz",
         "browser_download_url": "https://github.com/o/r/releases/download/v0.2.0/hurad-v0.2.0-x86_64-unknown-linux-musl.tar.gz",
         "size": 4},
        {"name": "SHA256SUMS",
    ...
    fn checksums_are_read_whichever_way_sha256sum_wrote_them() {
        let sums = "\
aaaa  hurad-v0.2.0-x86_64-unknown-linux-musl.tar.gz
bbbb *hurad-v0.2.0-aarch64-unknown-linux-musl.tar.gz
";
        assert_eq!(
            expected_sha(sums, "hurad-v0.2.0-x86_64-unknown-linux-musl.tar.gz").as_deref(),
            Some("aaaa")
        );
        assert_eq!(
            expected_sha(sums, "hurad-v0.2.0-aarch64-unknown-linux-musl.tar.gz").as_deref(),
            Some("bbbb")
        );
        // An asset the file does not cover is not an asset that gets installed.
        assert_eq!(
            expected_sha(sums, "hurad-v0.2.0-something-else.tar.gz"),
            None
        );
    }
    ...
    fn the_installer_the_workflow_and_the_updater_agree_on_the_names() {
        ...
        // `hura-$tag-$target.tar.gz`, built by `asset_name` here and by string
        // interpolation there.
        assert!(
            INSTALL_SH.contains("hurad-${tag}-${target}.tar.gz"),
            "install.sh names assets differently from asset_name"
        );
        assert!(
            RELEASE_WORKFLOW.contains("hurad-${TAG}-${{ matrix.target }}.tar.gz"),
            "the release workflow names assets differently from asset_name"
        );
        assert_eq!(
            asset_name("v0.2.0", "x86_64-unknown-linux-musl"),
            "hurad-v0.2.0-x86_64-unknown-linux-musl.tar.gz"
        );
        assert!(
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("hura-update-test-{name}-{}", std::process::id()));
    ...
    fn the_new_binary_lands_on_the_old_one_from_another_filesystem() {
        ...
        // Nothing left behind: a directory full of `.hurad-update-*` would be
        // the visible symptom of a swap that half-happened.
        let strays: Vec<_> = std::fs::read_dir(&installed)
            .unwrap()
            .filter_map(|e| e.ok())
            .filter(|e| e.file_name().to_string_lossy().starts_with(".hurad-update"))
            .collect();
    ...
    /// `hura` installed somewhere root owns.
    #[test]
    #[cfg(unix)]
    fn a_directory_that_cannot_be_written_says_what_to_do_about_it() {
        ...
        let fresh = downloaded.join("hura");
        std::fs::write(&fresh, "new").unwrap();
        let at = installed.join("hura");
        std::fs::write(&at, "old").unwrap();
    ...
    /// stale after a manual `hurad update`, or a downgrade -- must not be
