...
//! run where `hurad` is, and the skills live on the machine with the window on
//! it. A path in the server's config file cannot reach them.
//!
//! So the server keeps a **library** at `$XDG_DATA_HOME/hura/skills`, filled from
...
///
/// `$XDG_DATA_HOME/hura/skills`, beside the worktrees: data rather than state,
...
/// inside a skill directory is copied into the sandbox, and a `.hura-origin`
...
mod tests {
    ...
    fn a_skill(name: &str, files: &[(&str, &str)]) -> (PathBuf, PathBuf) {
        let root =
            std::env::temp_dir().join(format!("hura-skills-test-{}-{name}", std::process::id()));
