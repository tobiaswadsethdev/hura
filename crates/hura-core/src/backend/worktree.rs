...
//! shaped by saying so.** An authenticated `hurad` can already create containers
...
//! equivalent. Writing `.hura/` into the working copy would put it in every
...
//! other state, at `$XDG_STATE_HOME/hura/worktrees/<name>/`, and adoption after
...
//! agent is `hura-<name>` and its shells are `hura-<name>-shell-N`. Without that,
...
impl Worktree {
    ...
    /// `$XDG_DATA_HOME/hura/worktrees`, or `~/.local/share/hura/worktrees`.
    ///
    ...
    /// is a `repo` that is itself a path on this machine, which is what `hura new
    ...
    fn run(&self, argv: &[&str]) -> Result<ExecOutput> {
        ...
        let out = Command::new(bin)
            ...
            // in: hurad is long-lived and its working directory may have been
...
///
/// `hura-` prefixed for the same reason the sandbox names are: on a machine
...
impl Backend for Worktree {
    ...
    fn paths(&self, session: &Session) -> Paths {
        Paths {
            ...
            hura: self.record_dir(&session.name).display().to_string(),
        }
    }
    ...
    /// still set, because hurad's environment is not a login shell's and tmux
...
"#,
    ...
    /// come back: [`Self::fetch_script`] checks out an existing `hura/<name>`
...
mod tests {
    ...
    fn shells_are_named_per_session() {
        ...
        assert_eq!(b.shell_prefix(&one), "hura-alpha-shell-");
        assert_eq!(b.shell_prefix(&two), "hura-beta-shell-");
    ...
    fn the_record_lives_outside_the_worktree() {
        let tmp = std::env::temp_dir().join("hura-test-record");
        let b = backend(&tmp);
    ...
    fn the_fetch_script_is_idempotent() {
        let tmp = std::env::temp_dir().join("hura-test-fetch");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(tmp.join(".git")).unwrap();
        let b = backend(&tmp);
    ...
    fn checkout_with_a_session(tag: &str) -> (PathBuf, Worktree, Session) {
        let tmp = std::env::temp_dir().join(format!("hura-test-{tag}-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
    ...
    fn removing_a_session_that_did_no_work_takes_its_branch_with_it() {
        ...
        assert!(
            !branches(&repo).contains("hura/alpha"),
