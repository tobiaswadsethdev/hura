...
//! in somebody's notes. Now `hurad` owns the containers, so it has to own the
//! secrets too.
//!
...
/// `$XDG_STATE_HOME/hura/secrets.json`, beside the tokens and the key.
pub fn path() -> PathBuf {
...
///
/// `pub(crate)` deliberately. It is not reachable from `hurad`, from the CLI or
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let p =
            std::env::temp_dir().join(format!("hura-secrets-{name}-{}.json", std::process::id()));
