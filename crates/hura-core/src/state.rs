...
//!
//! `$XDG_STATE_HOME/hura`, falling back to `~/.local/state/hura` -- deliberately
//! not `$XDG_CONFIG_HOME/hura`, where the session cache and `config.toml`
...
/// `$XDG_STATE_HOME/hura`, or `~/.local/state/hura`.
#[cfg(unix)]
pub fn dir() -> PathBuf {
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let p =
            std::env::temp_dir().join(format!("hurad-state-test-{name}-{}", std::process::id()));
    ...
    fn xdg_state_home_wins_when_it_is_set() {
        ...
        assert_eq!(dir(), PathBuf::from("/tmp/statehome/hura"));
        match before {
