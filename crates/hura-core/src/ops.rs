    ...
    if repair {
        ...
        for s in stuck {
            ...
            let (state, note) = match seed::seed_state(backend, &s) {
                ...
                // Nothing to read: a sandbox seeded by an older hura, before the
...
///
/// The one description of a new session shared by `hura new` and the TUI's
...
pub fn create(
        ...
        Some(s) if s.state == State::Failed => {
            return Err(format!(
                "session `{0}` already exists, left by a create that failed; \
                 remove it with `hurad rm {0}` and try again",
                draft.name
            ));
        }
    ...
    // labelled `hura.managed`, with no record and no `meta.json` yet. A refresh
    // landing in that window -- the TUI runs one a second, and a `hura new` in
    // another terminal is the normal case -- tries to adopt it, and reports
    // `could not adopt hura-x: cat: /sandbox/.hura/meta.json: No such file or
...
///
/// Shared by `hura rm` and the TUI, so destroying means one thing wherever it is
...
mod tests {
    ...
    /// has to be the same one `hura new` uses -- which is why it lives here and
    /// not in a client.
    #[test]
    ...
    /// and first, because it is what `hura new` would have used.
    #[test]
    fn a_configured_policy_path_is_offered_ahead_of_the_templates() {
        let choices = policy_choices("/etc/hura/mine.yaml");
        assert_eq!(choices[0].spec, "/etc/hura/mine.yaml");
    ...
    fn a_new_session_carries_its_project_into_the_draft() {
        ...
        // And `hura new`, which has no projects, leaves it unset rather than
        // inventing one.
