...
mod tests {
    ...
    fn a_branch_line_carries_the_upstream_and_the_divergence() {
        ...
        parse_branch("hura/x...origin/hura/x [ahead 2, behind 3]", &mut s);
        assert_eq!(s.branch, "hura/x");
        assert_eq!(s.upstream.as_deref(), Some("origin/hura/x"));
    ...
    fn a_branch_with_no_upstream_says_so() {
        let mut s = Status::default();
        parse_branch("hura/new", &mut s);
        assert_eq!(s.branch, "hura/new");
