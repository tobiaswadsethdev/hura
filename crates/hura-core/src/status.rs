...
//! `/sandbox/.hura/status.json` on turn and tool boundaries. That gives clean
...
/// The record `hura-status` writes inside the sandbox.
///
/// Field names are a contract with `images/hura-base/hura-status`; `image.rs` has
...
mod tests {
    ...
    fn the_input_box_is_not_found_where_there_is_none() {
        ...
        assert!(!has_input_box("sandbox@hura:/sandbox/repo$ "));
    ...
    fn an_empty_pane_says_nothing() {
        ...
        assert_eq!(scrape_pane("sandbox@hura:/sandbox/repo$ "), None);
    }
